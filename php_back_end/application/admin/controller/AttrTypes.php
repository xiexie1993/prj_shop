<?php
// +----------------------------------------------------------------------
// | Description: 租赁方案类目 下 商品规格 属性值 控制器
// +----------------------------------------------------------------------
...
class AttrTypes extends ApiCommon
{
    // 【 租赁方案类目 下 商品规格 属性值  】查询出列表
    public function listdata()
    {
        $param = $this->param;
        $objModel = model('AttrType');
        $map['status'] = 1;
        ...
        {
            $debug['title'] = '【 租赁方案类目 下 商品规格 属性值  】列表查询';
            $debug['param'] = $param;
    ...
    public function newadd()
    {
        ...
        else
        {
            ...
            else
            {
                ...
                $log['content']     = "新增了 租赁方案类目 下 商品规格 属性值 ";
        ...
        if(config('api_debug_enabled')==1)//开启调试输出调试参数
        {
            $debug['title'] = '【 租赁方案类目 下 商品规格 属性值  】新增';
            $debug['param'] = $param;
            $ret_data['debug'] = $debug;
        }
        return json($ret_data);
    ...
    // 【 租赁方案类目 下 商品规格 属性值  】修改/编辑
    public function edit()
    {
        $param = $this->param;
        if(
              ($param['attr_id'] != null && $param['attr_id'] !='')
            // ||($param['category_id'] != null && $param['category_id'] !='')
            ||($param['category_att_id'] != null && $param['category_att_id'] !='')
          ...
          )
        {
            ...
            else
            {
                ...
                $log['uid'] = $userInfo['id'];
                $log['username']    = $userInfo['username'];
                $log['content']     = "编辑了 租赁方案类目 下 商品规格 属性值 ";
                $log['create_time'] = time();
                $OpLogModel->createData($log);
                // 记录操作日志 End
                $ret_data['status'] = 1;
        ...
        if(config('api_debug_enabled')==1)//开启调试输出调试参数
        {
            $debug['title'] = '【 租赁方案类目 下 商品规格 属性值  】修改/编辑';
            $debug['param'] = $param;
            $ret_data['debug'] = $debug;
        }
        return json($ret_data);
    ...
    public function vdel()
    {
          ...
          )
        {
            ...
            else
            {
                ...
                $log['uid'] = $userInfo['id'];
                $log['username']    = $userInfo['username'];
                $log['content']     = "删除了一个 租赁方案类目 下 商品规格 属性值 ";
                $log['create_time'] = time();
                $OpLogModel->createData($log);
                // 记录操作日志 End
                $ret_data['status'] = 1;
        ...
        if(config('api_debug_enabled')==1)//开启调试输出调试参数
        {
            $debug['title'] = '【 租赁方案类目 下 商品规格 属性值  】软删除';
            $debug['param'] = $param;
            $ret_data['debug'] = $debug;
        }
        return json($ret_data);
