$(function(){
  var tableMes = {
    urlData: urlLink('goodsOptionCategory'),
    columns:[
...
function operateFn(id){
  ...
  return {
    ...
    change: function() {
        //alert(that.data);
      var data = ajaxMethod('goodsOptionCategoryShowone',that.data);
      console.log(data);
    ...
    // power: function() {
    //   var data = ajaxMethod('goodsOptionCategoryTypeId',{roleId:that.data.id});
...
function sure (id) {
  switch(operateStr){
    case "new":
      ...
      formData.enable = '1';
      var result = ajaxMethod('goodsOptionCategoryAdd',formData);
    ...
    case "change":
      ...
      var result = ajaxMethod('goodsOptionCategoryEdit',formData);
       console.log(result);
      if(result.status != '0'){
        $('#roleMes').modal('hide');
        bstpTable.inint({});
    ...
    case "del":
      ...
      var result = ajaxMethod('goodsOptionCategoryDel',data);
      if(result.status != '0'){
        $('#typeDel').modal('hide');
        bstpTable.inint({});
        layerMes('操作成功');
