$(function(){
  var tableMes = {
    ...
    columns:[
      ...
      {field: 'remark',title: '客户备注',align: 'center',valign: 'middle'},
       ...
       {field: 'order_status',title: '订单状态',align: 'center',valign: 'middle',color:'red',formatter: function (value, row, index) {
     var element = "<span style='color:red'>"
                  +typearry.order[row.order_status]
                  +"</span>";
     return element;
      }},
 ...
 // bstpTable.inint({});
   bstpTable.inint({order_status:'2'});
  ...
  switch(operateStr){
    ...
    case "change":
     var formData = formJson($('#roleMes form:first').serializeArray());
      formData.order_id = $('#roleMes').attr('data-id');
      formData.order_status = '3';
      console.log(formData);
    
      var result = ajaxMethod('orderEdit',formData);
      if(result.status != '0'){
        $('#roleMes').modal('hide');
          bstpTable.inint({order_status:'2'});
        layerMes('操作成功');
