$(function()
{
  ...
  var tableMes = {
        ...
        columns:[
                  ...
                  {field: 'category_id',title: '规格所属类目',align: 'center',valign: 'middle'},
...
function operateFn(id){
  var that = this;
  that.data = {type_id:id};
  //alert(that.data);
  return {
    new: function() {
      $('#myModal').modal({backdrop: 'static'});
      // addChange('new'); 
      var goodsCategorys = ajaxMethod('leaseallType');
      // console.log(goodsCategorys);
      // console.log(goodsCategorys);
      // 组装 下拉分类选择框
      var rowsData = goodsCategorys.rows;
    ...
    change: function() {
      //alert(that.data);
      var data = ajaxMethod('categoryTypeShowone',that.data);
      var goodsCategorys = ajaxMethod('leaseallType');
      // console.log(data);
      // console.log(goodsCategorys);
      // 组装 下拉分类选择框
    ...
    del: function()
    {
      ...
      $('#typeDel').attr('data-id',that.data.type_id);
    }
  }
}
//数据刷一遍
...
function sure (id) {
  switch(operateStr)
  {
    case "new":
      ...
      formData.category_id = $('#addlCategorydiv1 option:selected').val();
      // formData.enable = '1';
      console.log(formData);
      var result = ajaxMethod('categoryTypeAdd',formData);
      console.log(result);
    ...
    case "change":
      ...
      formData.category_id = $('#lCategorydiv option:selected').val();
      // console.log(formData);
      var result = ajaxMethod('categoryTypeEdit',formData);
      // console.log(result);
      if(result.status != '0'){
        $('#roleMes').modal('hide');
        bstpTable.inint({});
    ...
    case "power":
      ...
      break;
    case "del":
      var data = {type_id:$('#typeDel').attr('data-id')};
      var result = ajaxMethod('categoryTypeDel',data);
      if(result.status != '0'){
        $('#typeDel').modal('hide');
        bstpTable.inint({});
        layerMes('操作成功');
